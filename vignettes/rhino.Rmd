...
This tutorial shows an easy way of implementing intrnationalization in a Shiny application
built using [Rhino framework](https://appsilon.github.io/rhino/).
The goal is to recreate a simple application that can be [found in the
...
First step is to create a Rhino application.
...
After seting the initial application, the next step is to install `shiny.i18n` and add it to the dependencies.

_NOTE: `shiny.i18n` compatibility with `rhino` is currently available only in the development varsion. The installation guide will be updated after cRAN release._
...
Next step is to fill the application with the content.
...
First, lets implement the basic version of the app (without internationalization).
...
of "Old Faithful Geyser Data" app.
...
Next step is to add an instance of `shiny.i18n::Translator` class and provide the translations.
In `shiny.i18n`, translations can be added as csv or json file - we will use the latter.
...
The next step is to choose the default translation:
...
We also need to add `shiny.i18n` dependencies and add translator to the UI
by calling `usei18n()`:
...
## Translate server part

The only missing part is the plot - the title and y axis label.
The text there is rendered in the server, so we need to add translator there:
...
```
Now, if you run the application, everything will be translated:
...
A Rhino application is usually build from multiple Shiny modules.
...
In this section we are going to show how the translator can used in such a setup,
...
Important part to notice here is that the Translator (`i18n` object) needs
to be passed as argument to inner module.

Next step is to call the main panel module in the `app/main.R` file:
...
The translation mechanism works, but the application code is now
better structured.
